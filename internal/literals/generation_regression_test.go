...
func TestByteSliceDecoderCapacityMatchesLiteral(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			checkByteSliceDecoderCapacity(t, tc.obf, data)
		})
	}
}

// checkByteSliceDecoderCapacity inspects the decoder before its call-site
// wrapper is chosen. The capacity contract is independent of whether that
// wrapper is a closure or a lifted top-level function.
func checkByteSliceDecoderCapacity(t *testing.T, obf obfuscator, data []byte) {
	t.Helper()
	rnd := mathrand.New(mathrand.NewSource(1))
	decoder := obf.obfuscate(
		rnd,
